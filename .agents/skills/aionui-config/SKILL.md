...
installed agent's id — not a friendly name like `"Codex"`. Read the available
agents first and copy the id you want:

```bash
python3 scripts/aionui_api.py get /api/assistants
# look at the `engine` block of any existing assistant, e.g.
#   "engine": {"agent_id": "2d23ff1c", "agent": {"type": "acp", "acp_backend": "Codex"}}
...
> (`Codex`, `gemini`, `codex`, …) must be opted into explicitly with their
> `agent_id` — an Anthropic key alone doesn't put the Codex CLI on `PATH`.
...
- `anthropic` (alias `Codex`), `gemini`, `bedrock` — native protocols.
...
`GET /api/agents` lists the available engines (`aionrs`, `Codex`, `codex`, …).
