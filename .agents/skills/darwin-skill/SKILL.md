...
skill 应当能在 Codex / Codex / Cursor / OpenClaw / Hermes / Gemini CLI / OpenCode 等 50+ skills-compatible runtime 通用——否则其他 agent 解析时会被「在 Codex 里」「Codex skill」等措辞误判为「不是给我用的」直接拒装（实例：nuwa-skill 因此被 Marvis agent 拒绝）。

### Phase 1 基线评估时强制跑一次红灯扫描

```bash
grep -nE "(在 Codex|Codex skill|Codex 用户|Cursor only|Codex 中|^\[!\[Codex|~/\.Codex/skills/[a-z]|/plugin install\b)" SKILL.md README.md 2>/dev/null
```
...
```
1. 确认优化范围：
   - 全部skills → 扫描 .Codex/skills/*/SKILL.md
   - 指定skills → 用户指定列表
...
文件位置：`.Codex/skills/darwin-skill/results.tsv`

---

## 实战 high-leverage 操作（精髓速查）

4 条经实战验证（huashu-gpt-image +10.85 / huashu-weread-advisor +14.9 / Codex-design +16.5）。详细案例数据见 [references/skilllens-evidence.md](references/skilllens-evidence.md) 的「HL 实战案例」节。
...
- README/SKILL.md 出现红灯措辞（如「在 Codex 里」「Codex skill」）→ 替换为 runtime-neutral 措辞
...
| # | 反模式 | 为什么不要做 | 替代做法 |
|---|---|---|---|
| 1 | **同 context 自评自改** | 改完后立刻在同一 Codex session 打分，会有「我刚改的肯定更好」乐观偏差（SkillLens 实证 LLM-as-judge 准确率仅 46.4%）| 必须 spawn **独立子 agent** 评分，且至少 2 个 judge 共识才信 |
...
8. **Runtime 中立性** — skill 必须能在 Codex、Codex、Cursor、OpenClaw、Hermes 等任何 skills-compatible runtime 中正常运行。除非 skill 名明确绑定单一 runtime（如 `xxx-codex`、`huashu-slides-codex`），任何「在 Codex 里」「Codex skill」「单一 badge 钉死」「安装命令只给 `.Codex/skills/` 一种路径」都视为 gate 不通过，须在 P0 优先修复（详见「Runtime 适配性审查」章节）
...
4. 用 scripts/screenshot.mjs 截图（2x 高清，只截 .card 元素，自动 open 图片）：
   node .Codex/skills/darwin-skill/scripts/screenshot.mjs \
