...
function formatWorkflowNodeConfig(node) {
  if (!node) return '';
  if (node.type === 'skill') return getWorkflowSkillMeta(node.skillId)?.label || '璇佹嵁鍖呮暣鐞?;
  if (node.type === 'condition') {
    const metric = WORKFLOW_CONDITION_METRICS.find(item => item.id === node.conditionMetric)?.label || node.conditionMetric || '璧勮鏁伴噺';
    return metric + ' ' + (node.conditionOperator || '>=') + ' ' + (node.conditionValue || 1);
  }
  if (node.type === 'classifier') return '鍒嗙被妗讹細' + (node.classifierLabels || '蹇呰,杩借釜,绱犳潗,鍒涗綔,闄嶅櫔');
  return '';
}
