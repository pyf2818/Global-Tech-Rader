...
/* 定时任务区块：基于 useAgentJobs hook，与当前 agent 绑定
 * - 列表：展示该用户所有任务，按下次执行时间升序
 * - 新建：cron + 任务目标 prompt
 * - 启停/删除：单条操作
 * - 预览：实时计算下次执行时间 */
function AgentJobsSection({ agent }) {
  const { jobs, loading, error, createJob, updateJob, deleteJob, toggleJob, previewCron } = useAgentJobs();
